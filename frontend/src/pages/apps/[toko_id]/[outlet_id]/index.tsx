...
import {IPages, Outlet} from '@type/index'
...
export default function OutletIndex({meta}: IPages<Outlet>) {
  ...
  const {data,error}=useSWR<IResponse>(`/outlets/${outlet_id}/insight?filter=${query?.filter}${query?.filter === 'custom' ? `&from=${query?.from}&to=${query?.to}` : ''}`)
  ...
  React.useEffect(()=>{
    if(data) {
        setIData({income_today:Number(data.data.today.income),tr_today:Number(data.data.today.total_transactions),income:Number(data.data.data.income),tr:Number(data.data.data.total_transactions)})
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Header title={meta?.data.name} desc={meta?.data.description}>
        <Dashboard title={meta?.data.name} subtitle={meta?.data?.toko?.name} view='dashboard'>
          <Container>
            <Box pb={2} mb={5}>
