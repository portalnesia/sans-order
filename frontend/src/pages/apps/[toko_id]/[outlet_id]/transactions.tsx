...
export const getServerSideProps = wrapper({name:'check_outlet',outlet:{onlyMyToko:true,onlyAccess:['Transaction']},translation:'dash_tr'})
...
export function TableTr({data}: {data: Transaction}) {
  const {t} = useTranslation('dash_tr');
...
export default function OutletTransactions({meta}: IPages<Outlet>){
  const {t} = useTranslation('dash_tr');
  const {t:tMenu} = useTranslation('menu');
  const {t:tCom} = useTranslation('common');
  const router = useRouter();
  ...
  const {data,error} = useSWR<Transaction,true>(`/transactions/outlet/${outlet_id}?page=${page}&pageSize=${rowsPerPage}&filter=${query?.filter}${query?.filter === 'custom' ? `&from=${query?.from}&to=${query?.to}` : ''}`)
  ...
  const handleDownload=React.useCallback(async()=>{
    setLoading('download')
    try {
      const url = await get<string>(`/transactions/outlet/${outlet_id}/report?filter=${query?.filter}${query?.filter === 'custom' ? `&from=${query?.from}&to=${query?.to}` : ''}`);
      const urlEdit = `${process.env.NEXT_PUBLIC_API_URL}${url.data}`
  ...
  },[query,get,setNotif,outlet_id,tCom])

  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Header title={`${tMenu("transactions")} - ${meta?.data?.name}`} desc={meta?.data?.description}>
        <Dashboard title={meta?.data?.name} subtitle={meta?.data?.toko?.name} view='dashboard transactions'>
          <Container>
            <Box pb={2} mb={5}>
