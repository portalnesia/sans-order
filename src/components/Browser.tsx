...
import {IFiles,ResponsePagination} from '@type/index'
import {uuid,number_size} from '@portalnesia/utils'
...
function BrowserComp(props: BrowserProps) {
    ...
    const {onSelected,open,scroll='body',maxWidth='md',fullWidth=true,onClose} = props;
    ...
    const {data,error,mutate} = useSWR<ResponsePagination<IFiles>>(open ? `/files/browser?per_page=30&page=${page||1}` : null);
    ...
    const handleUpload = React.useCallback((e: React.DragEvent<HTMLDivElement>|React.ChangeEvent<HTMLInputElement>)=>{
        ...
        if(file) {
            if(/image\/(jpg|jpeg|png)/.test(file.type)) {
                ...
                upload<IFiles>('/files/browser',form,opt)
                .then(()=>{
                    //const a = data;
                    //a.unshift(res);
                    //setData(a);
    ...
    },[data,setNotif,upload,disabled,mutate,serverType])
            ...
            {open && (
                <React.Fragment>
                    <Dialogs classes={{scrollBody:`file-manager-${idScroll.current}`}} open={true} scroll={scroll} maxWidth={maxWidth} fullWidth={fullWidth}>
                        ...
                        <DialogActions>
                            <Pagination count={Number(data?.total_page||1)} page={Number(page||1)} onChange={setPage} />
                        </DialogActions>
                    </Dialogs>
