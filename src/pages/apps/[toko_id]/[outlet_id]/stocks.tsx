...
export default function OutletStocks({meta}: IPages) {
  ...
  const handleCreate=React.useCallback(async(e?: React.FormEvent<HTMLFormElement>)=>{
    ...
    try {
      ...
      await post(`/toko/${toko_id}/${outlet_id}/stocks`,{...input,item_id:Number(input.item_id),recaptcha});
      mutate();
  ...
  const handleEdit=React.useCallback(async(e?: React.FormEvent<HTMLFormElement>)=>{
    ...
    try {
      const recaptcha = await captchaRef.current?.execute();
      await put(`/toko/${toko_id}/${outlet_id}/stocks/${dEdit?.id}`,{...input,item_id:Number(input.item_id),recaptcha});
      mutate();
      setNotif(tCom("saved"),false)
      setDEdit(null)
    } catch(e: any) {
  ...
  const handleAutocompleteInputChange=React.useCallback((e: React.SyntheticEvent<Element, Event>, value: string,reason: AutocompleteInputChangeReason)=>{
    if(reason==='input') {
      ...
      if(filter?.length === 0){
        setIngLoading(true)
        get<ResponsePagination<IIngredients>>(`/toko/${toko_id}/${outlet_id}/ingredients?per_page=15&q=${encodeURIComponent(`${value}`)}`)
        .then((res)=>{
          let b=ingOptions;
  ...
  React.useEffect(()=>{
    if(ingOptions.length === 0) {
      setIngLoading(true)
      get<ResponsePagination<IIngredients>>(`/toko/${toko_id}/${outlet_id}/ingredients?per_page=100`)
      .then((res)=>{
        setIngOption(res.data);
      }).catch((err)=>{
