...
export default function OutletIngredients({meta}: IPages) {
  ...
  const {data,error,mutate} = useSWR<ResponsePagination<IIngredients>>(`/toko/${toko_id}/${outlet_id}/ingredients?page=${page}&per_page=${rowsPerPage}`);
  ...
  const handleCreate=React.useCallback(async(e?: React.FormEvent<HTMLFormElement>)=>{
    ...
    try {
      ...
      await post(`/toko/${toko_id}/${outlet_id}/ingredients`,{...input,recaptcha});
      mutate();
  ...
  const handleEdit=React.useCallback(async(e?: React.FormEvent<HTMLFormElement>)=>{
    ...
    try {
      const recaptcha = await captchaRef.current?.execute();
      await put(`/toko/${toko_id}/${outlet_id}/ingredients/${dEdit?.id}`,{...input,recaptcha});
      mutate();
      setNotif(tCom("saved"),false)
      setDEdit(null)
    } catch(e: any) {
  ...
  const handleDelete=React.useCallback(async()=>{
    if(typeof dDelete !== 'boolean' && dDelete && 'id' in dDelete) {
      setLoading(true);
      try {
        await del(`/toko/${toko_id}/${outlet_id}/ingredients/${dDelete?.id}`);
        mutate();
  ...
  const handleDeleteAll=React.useCallback(async()=>{
    if(typeof dDelete === 'boolean') {
      ...
      try {
        ...
        await post(`/toko/${toko_id}/${outlet_id}/bulk/delete`,{type:'ingredient',ids});
        mutate();
